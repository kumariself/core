...
internal class AnalyticsDatasource(
    ...
    suspend fun queryTopArtistsLastXDays(x: Int) = databaseDao.queryTopArtistsInRange(
        startTimestamp = now().beforeXDays(x),
        endTimestamp = now()
    )
    ...
    suspend fun queryTopAlbumsLastXDays(x: Int) = databaseDao.queryTopAlbumsInRange(
        startTimestamp = now().beforeXDays(x),
        endTimestamp = now()
    )
    ...
    suspend fun getTotalPlaybackEventCount(): Long =
        databaseDao.getTotalPlaybackEventCount()

    suspend fun getTotalEventArtistCount(): Long =
        databaseDao.getTotalEventArtistCount()

    suspend fun getTotalListeningTimeInSeconds(): Long =
        databaseDao.getTotalListeningTimeInSeconds()
}