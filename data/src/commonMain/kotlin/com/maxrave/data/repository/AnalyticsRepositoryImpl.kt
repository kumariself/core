...
internal class AnalyticsRepositoryImpl(
    ...
    override suspend fun insertPlaybackEvent(
        ...
        emit(analyticsDatasource.insertPlaybackEvent(
            videoId,
            channelIds,
            albumBrowseId,
            durationSecond,
    ...
    override suspend fun queryTopPlayedSongsLastXDays(x: Int): Flow<List<TopPlayedTracks>> = flow {
    ...
    ): Flow<List<TopPlayedTracks>> = flow {
        emit(analyticsDatasource.queryTopPlayedSongsInRange(
            startTimestamp,
            endTimestamp
        ))
    }.flowOn(Dispatchers.IO)

    override suspend fun queryTopArtistsLastXDays(x: Int): Flow<List<TopPlayedArtist>> = flow {
    ...
    override suspend fun queryTopArtistsInRange(
        startTimestamp: LocalDateTime,
        endTimestamp: LocalDateTime
    ): Flow<List<TopPlayedArtist>> = flow {
        emit(analyticsDatasource.queryTopArtistsInRange(
            startTimestamp,
            endTimestamp
        ))
    }.flowOn(Dispatchers.IO)

    override suspend fun queryTopAlbumsLastXDays(x: Int): Flow<List<TopPlayedAlbum>> = flow {
    ...
    override suspend fun queryTopAlbumsInRange(
        startTimestamp: LocalDateTime,
        endTimestamp: LocalDateTime
    ): Flow<List<TopPlayedAlbum>> = flow {
        emit(analyticsDatasource.queryTopAlbumsInRange(
            startTimestamp,
            endTimestamp
        ))
    }.flowOn(Dispatchers.IO)

    override suspend fun getTotalPlaybackEventCount(): Flow<Long> = flow {
        emit(analyticsDatasource.getTotalPlaybackEventCount())
    }.flowOn(Dispatchers.IO)

    override suspend fun getTotalEventArtistCount(): Flow<Long> = flow {
        emit(analyticsDatasource.getTotalEventArtistCount())
    }.flowOn(Dispatchers.IO)

    override suspend fun getTotalListeningTimeInSeconds(): Flow<Long> = flow {
